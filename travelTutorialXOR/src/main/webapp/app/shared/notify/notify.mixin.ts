...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-traveltutorialxorapp-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-traveltutorialxorapp-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-traveltutorialxorapp-error'], {
        param: decodeURIComponent(error.response.headers['x-traveltutorialxorapp-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-traveltutorialxorapp-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-traveltutorialxorapp-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-traveltutorialxorapp-alert'], {
      param: decodeURIComponent(response.headers['x-traveltutorialxorapp-params'].replace(/\+/g, ' ')),
    });
  }
}
