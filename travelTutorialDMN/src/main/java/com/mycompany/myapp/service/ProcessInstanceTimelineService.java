...
public class ProcessInstanceTimelineService {
    ...
    public ProcessInstanceTimelineDTO getTimeline(Long processInstanceId) {
        ...
        ProcessInstanceTimelineGroupDefinitionDTO timelineGroupDefinition = processInstanceTimelineDefinitionService.getTimelineGroup(processInstance.getProcessDefinition().getId());
    ...
    private ProcessInstanceTimelineDefinitionDTO findTimelineDefinition(ProcessInstance processInstance, ProcessInstanceTimelineGroupDefinitionDTO timelineGroupDefinition) {
    ...
    private ProcessInstanceTimelineDTO calculateTimeline(ProcessInstance processInstance, ProcessInstanceTimelineDefinitionDTO timelineDefinition) {
        ProcessInstanceTimelineDTO timeline = new ProcessInstanceTimelineDTO();
        timeline.setTitle(timelineDefinition.getName());

        timelineDefinition.getItems().forEach(timelineItemDefinition -> {
            timeline.getItems().add(new ProcessInstanceTimelineItemDTO()
    ...
    private void calculateStatus(ProcessInstance processInstance, ProcessInstanceTimelineItemDTO timelineItem) {
        if (expressionService.evaluateCompleteStatusExpression(processInstance, timelineItem.getItemDefinition().getCheckStatusExpression())) {
            timelineItem.setStatus("COMPLETED");
        }
    }

    private void calculateStatusWithoutParameter(ProcessInstance processInstance, ProcessInstanceTimelineItemDTO timelineItem) throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {
        Method checkStatusMethod = ProcessInstanceTimelineService.class.getMethod(timelineItem.getItemDefinition().getCheckStatusExpression(), ProcessInstance.class, ProcessInstanceTimelineItemDTO.class);
        checkStatusMethod.invoke(this, processInstance, timelineItem);
    }

    private void calculateStatusWithParameters(ProcessInstance processInstance, ProcessInstanceTimelineItemDTO timelineItem) throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {
        ...
        Method checkStatusMethod = ProcessInstanceTimelineService.class.getMethod(timelineItem.getItemDefinition().getCheckStatusExpression(), ProcessInstance.class, ProcessInstanceTimelineItemDTO.class, String.class);
