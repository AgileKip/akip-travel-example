<template>
  <div>
    <h2 class="jh-entity-heading" data-cy="travelPlanProcessDetailsHeading">
      <span v-text="$t('travelTutorialCallApp.travelPlanProcess.home.title')">TravelPlanProcess</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('travelTutorialCallApp.travelPlanProcess.home.refreshListLabel')">Refresh List</span>
        </button>

        <router-link :to="`/process-definition/TravelPlanProcessCALL/init`" tag="button" class="btn btn-primary mr-2">
          <font-awesome-icon icon="plus"></font-awesome-icon>
          <span v-text="$t('travelTutorialCallApp.travelPlanProcess.home.createLabel')"> Create a new Travel Plan Process Instance </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && travelPlanProcessList && travelPlanProcessList.length === 0">
      <span v-text="$t('travelTutorialCallApp.travelPlanProcess.home.notFound')">No travelPlanProcess found</span>
    </div>
          ...
          <tr v-for="travelPlanProcess in travelPlanProcessList" :key="travelPlanProcess.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="travelPlanProcess.processInstance">
                <router-link :to="`/process-definition/TravelPlanProcessCALL/instance/${travelPlanProcess.processInstance.id}/view`">
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link
                  :to="`/process-definition/TravelPlanProcessCALL/instance/${travelPlanProcess.processInstance.id}/view`"
                  tag="button"
