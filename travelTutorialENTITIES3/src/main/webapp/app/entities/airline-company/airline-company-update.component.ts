    ...
    if (this.airlineCompany.id) {
      this.airlineCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntities3App.airlineCompany.updated', { param: param.id });
      ...
      this.airlineCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntities3App.airlineCompany.created', { param: param.id });
