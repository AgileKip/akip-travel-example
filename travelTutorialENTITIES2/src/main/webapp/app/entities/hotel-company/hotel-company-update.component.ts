    ...
    if (this.hotelCompany.id) {
      this.hotelCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntities2App.hotelCompany.updated', { param: param.id });
      ...
      this.hotelCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntities2App.hotelCompany.created', { param: param.id });
