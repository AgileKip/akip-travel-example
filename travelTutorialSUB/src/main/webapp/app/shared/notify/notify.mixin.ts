...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-traveltutorialsubapp-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-traveltutorialsubapp-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-traveltutorialsubapp-error'], {
        param: decodeURIComponent(error.response.headers['x-traveltutorialsubapp-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-traveltutorialsubapp-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-traveltutorialsubapp-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-traveltutorialsubapp-alert'], {
      param: decodeURIComponent(response.headers['x-traveltutorialsubapp-params'].replace(/\+/g, ' ')),
    });
  }
}
