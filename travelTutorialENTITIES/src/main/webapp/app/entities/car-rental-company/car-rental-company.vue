<template>
  <div>
    <h2 id="page-heading" data-cy="CarRentalCompanyHeading">
      <span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.home.title')" id="car-rental-company-heading">Car Rental Companies</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CarRentalCompanyCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.home.createLabel')"> Create a new Car Rental Company </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && carRentalCompanies && carRentalCompanies.length === 0">
      <span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.home.notFound')">No carRentalCompanies found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="carRentalCompanies">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.name')">Name</span></th>
            <th scope="row"><span v-text="$t('travelTutorialEntitiesApp.carRentalCompany.code')">Code</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="travelTutorialEntitiesApp.carRentalCompany.delete.question"
      ...
      <div class="modal-body">
        <p
          ...
          v-text="$t('travelTutorialEntitiesApp.carRentalCompany.delete.question', { id: removeId })"
        >
