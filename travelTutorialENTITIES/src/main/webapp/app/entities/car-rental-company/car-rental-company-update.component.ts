    ...
    if (this.carRentalCompany.id) {
      this.carRentalCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntitiesApp.carRentalCompany.updated', { param: param.id });
      ...
      this.carRentalCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialEntitiesApp.carRentalCompany.created', { param: param.id });
