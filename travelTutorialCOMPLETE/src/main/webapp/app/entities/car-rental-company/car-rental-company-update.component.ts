    ...
    if (this.carRentalCompany.id) {
      this.carRentalCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialCompleteApp.carRentalCompany.updated', { param: param.id });
      ...
      this.carRentalCompanyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('travelTutorialCompleteApp.carRentalCompany.created', { param: param.id });
